...
object PresidioRegexRecognizers {
    ...
    private val recognizers = listOf(
        Recognizer(PiiType.FINANCIAL, Regex("""\b(?:\d[ -]?){13,16}\b"""), 0.80f),
        Recognizer(PiiType.FINANCIAL, Regex("""\b\d{3}-\d{2}-\d{4}\b"""), 0.90f),
        Recognizer(PiiType.PHONE, Regex("""\+?\d[\d\s\-().]{6,14}\d"""), 0.70f),
        Recognizer(PiiType.EMAIL, Regex("""\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b"""), 0.95f),
        Recognizer(PiiType.MISC, Regex("""\b[A-Z]{1,2}\d{6,9}\b"""), 0.60f),
        Recognizer(PiiType.HEALTH, Regex("""\bMRN[:#\s-]*\d{5,10}\b""", RegexOption.IGNORE_CASE), 0.85f),
    )
