...
object SensitivityScorer {
    ...
    private const val LOCAL_THRESHOLD = 0.70f
    private const val REDACT_THRESHOLD = 0.35f

    fun compute(entities: List<PiiEntity>, signals: Set<Signal>, label: RequestLabel): Float =
        computeBreakdown(entities, signals, label).finalScore
    ...
    fun computeBreakdown(
        ...
        val final = ((entityScore + signalBoost) * multiplier).coerceIn(0.0f, 1.0f)
        val zone = when {
            final >= LOCAL_THRESHOLD -> "LOCAL"
            final >= REDACT_THRESHOLD -> "REDACT_THEN_CLOUD"
            else -> "CLOUD"
        }
        return ScoringBreakdown(entityScore, signalBoost, multiplier, final, zone)
    }
