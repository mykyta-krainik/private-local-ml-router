...
class ActionExecutor : ActionBackend {

    override fun execute(call: FunctionCall): ActionResult {
        val description = when (call.function) {
            "set_timer" -> "Set timer: ${call.args["amount"]} ${call.args["unit"]}(s)"
            "set_alarm" -> "Set alarm: ${call.args["hour"]}:${call.args["minute"].toString().padStart(2, '0')} ${call.args["meridiem"]}"
