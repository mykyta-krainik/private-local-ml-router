...
class PolicyConfigLoader {
    ...
    fun load(json: String): PolicyConfig = fromDto(adapter.fromJson(json)
    ...
    private fun fromDto(dto: Dto): PolicyConfig = PolicyConfig(
        ...
        entityRules = dto.entityRules.map {
            EntityRule(PiiType.valueOf(it.type), it.action, it.override)
        },
        signalRules = dto.signalRules.map {
            SignalRule(Signal.valueOf(it.signal), it.action, it.override)
        },
    ...
    internal data class ThresholdsDto(
        val local: Float,
        val redact_then_cloud: Float,
        val cloud: Float,
    )

    @JsonClass(generateAdapter = false)
    internal data class EntityRuleDto(
        val type: String,
        val action: String,
    ...
    @JsonClass(generateAdapter = false)
    internal data class SignalRuleDto(
        val signal: String,
        val action: String,
        val override: Boolean,
    )
